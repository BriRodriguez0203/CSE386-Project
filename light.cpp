...
color diffuseColor(const color& mat, const color& lightColor,
	...
	double dot = glm::dot(l, n);
	return mat * lightColor * glm::max(dot, 0.0);
}
...
color specularColor(const color& mat, const color& lightColor,
	double shininess,
	const dvec3& r, const dvec3& v) {
	double dot = glm::max(glm::dot(r, v), 0.0);
	double num = glm::pow(dot, shininess);
	return glm::clamp(mat * lightColor * num, 0.0, 1.0);
}
...
color totalColor(const Material& mat, const color& lightColor,
	...
	dvec3 aColor = ambientColor(mat.ambient, lightColor);
	dvec3 dColor = diffuseColor(mat.diffuse, lightColor, l, n);
	dvec3 sColor = specularColor(mat.specular, lightColor, mat.shininess, r, v);
	if (attenuationOn) {
		double d = glm::length(lightPos - intersectionPt);
		double at = 1 / (ATparams.constant + (ATparams.linear * d) + (ATparams.quadratic * glm::pow(d, 2)));
		return glm::clamp((aColor + at * (dColor + sColor)), 0.0, 1.0);
	} else {
		return glm::clamp((aColor + dColor + sColor), 0.0, 1.0);
	}
}
...
color PositionalLight::illuminate(const dvec3& interceptWorldCoords,
	...
	if (!isOn) {
		...
		return totalColor(material, lightColor, glm::normalize(eyeFrame.origin - interceptWorldCoords),
...
dvec3 PositionalLight::actualPosition(const Frame& eyeFrame) const {
	return isTiedToWorld ? eyeFrame.globalCoordToFrameCoords(pos) : eyeFrame.frameCoordsToGlobalCoords(pos);
}
...
bool PositionalLight::pointIsInAShadow(const dvec3& intercept,
	...
	OpaqueHitRecord hit;
	Ray ray = getShadowFeeler(intercept, normal, eyeFrame);
	VisibleIShape::findIntersection(ray, objects, hit);

	if (hit.t != FLT_MAX && glm::distance(intercept, hit.interceptPt) < glm::distance(intercept, eyeFrame.origin)) {
		return true;
	}
	return false;
}
...
Ray PositionalLight::getShadowFeeler(const dvec3& interceptWorldCoords,
	...
	dvec3 l = glm::normalize(actualPosition(eyeFrame) - interceptWorldCoords);
	Ray shadowFeeler(IShape::movePointOffSurface(interceptWorldCoords, normal), l);
	return shadowFeeler;
}
...
bool SpotLight::isInSpotlightCone(const dvec3& spotPos,
	...
	double cutoff = glm::cos(spotFOV / 2.0);
